"""Two-person variant of diag_auto_angles2.py that enriches only ONE of the
two people's reference angles (leaving the other as a single best-frontal
...
def main():
    ...
    ap.add_argument("--person", type=int, required=True, help="which person index to enrich")
    ...
    api.target_auto_angles({"person": args.person, "index": 0})
    print(f"[diag] TARGET_FACES now has {len(g.TARGET_FACES)} angle(s) total "
          f"(only person {args.person} enriched)", flush=True)
    ...
    g.execution_threads = int(g.CFG.max_threads or 8)
    ...
    out_dir = os.path.join(APP, "output", "auto_angles_diag_one")
    out, elapsed, face_log = run_swap(swap_video, facesets, list(g.TARGET_FACES),
                                       list(g.TARGET_FACE_GROUP), options, out_dir)
    print(f"[diag] swap with person-{args.person}-enriched targets -> {out} in {elapsed:.1f}s", flush=True)
