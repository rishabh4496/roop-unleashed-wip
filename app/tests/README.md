...
| `test_angle_handling.py` | The three shared angle layers. The polygon is placed by `swap_template_points`, **not** a hand-rolled `arcface_dst * size/112` — that guess is 53px out on a 512 crop and shipped once, because this test made the same guess. It **never trims real face** at any pose (deliberately stricter than the shipped, feathered trim), is off entirely near frontal, and is a pitch correction only — exactly 0 on a level head at any yaw. The trim is applied **after** the feather (before it, shrinking the matte shrank the feather 29px→18px and *sharpened* the seam). The fade is bounded, monotone, exactly off at 0, and fades a 1024/2048 enhancer surface without ghosting. Both paste call sites pass the polygon; the default path is bit-identical |
