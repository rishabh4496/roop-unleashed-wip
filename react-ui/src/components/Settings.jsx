    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 4xl:grid-cols-4 gap-6">
        ...
        <FilterSection title="Advanced performance (restart to apply)" query={query}>
          ...
          <Select label="Detect/Mask pool" info="ROOP_DETMASK_POOL — TensorRT contexts for face detection and masking. This one also sets how many detections run concurrently during 'Analyzing faces', which is ~85% detection — so LOWERING it slows that stage almost proportionally. Same VRAM tiers as the swapper pool (12GB card = 4). If 'detect' dominates the terminal's STAGE TIMING breakdown your threads are queueing for a free instance: raise this toward Max threads (each instance loads its own small model set)." value={p.perf_detmask_pool || 'auto'} onChange={(v) => set('perf_detmask_pool', v)} options={meta.pool_sizes || ['auto', '1', '2', '3', '4', '5', '6', '7', '8']} />
