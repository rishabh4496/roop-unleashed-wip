    ...
    <div className="flex flex-col lg:flex-row gap-6 items-start w-full">
      ...
      <div className={`w-full lg:w-[380px] 3xl:w-[440px] 4xl:w-[520px] shrink-0 pr-0 lg:pr-2 space-y-5 select-none ${!showLeftPanel ? 'hidden' : ''}`}>
        ...
        <div className="space-y-5">
          ...
          <Toggle label="↔️ Angle: trim hidden surface" info="LEAVE THIS OFF. It was meant to stop a profile pasting swap pixels over hair and neck, by projecting a reference head at the solved pose and trimming the paste mask to the surface still facing the camera. Measured against the real mask and split by region, everything it actually removed was FOREHEAD — 0.0% below the brow line at every pose tested, up to 8.1% above it. That is the &quot;only the middle of the face is swapped, the rest keeps the original texture&quot; report, and the seam sits across the brow. The reason is that the forehead is the one part of the face where nothing has evidence: the 106-point landmarks stop at the eyebrows, so the paste mask extrapolates upward by 0.6 of the brow-chin distance and this polygon extrapolates by carrying a reference head past the brows — and the difference between two guesses at the same unknown was being cut out of your face. Its top edge is now opened up, which keeps the left/right boundary (the real information) and discards its opinion about how high the face goes; on test geometry that takes the whole layer to 0.0% at every pose. So everything it was measurably doing was the artefact, and what is left is unproven. It is also placed from the same pose solve as the alignment selector above, which is 15-20° wrong on a face whose nose differs from the reference head, and it is capped so it can never remove more than a quarter of the mask. " checked={!!p.angle_visibility_mask} onChange={(v) => set('angle_visibility_mask', v)} />
