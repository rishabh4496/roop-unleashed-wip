...
export default function FaceSwap({
  ...
  const buildPreviewPayload = (params, { index, frame: fr, fake, ...overrides } = {}) => {
    ...
    return {
      ...
      angle_visibility_mask: activeParams.angle_visibility_mask,
      angle_fade_strength: activeParams.angle_fade_strength,
    ...
    <div className="flex flex-col lg:flex-row gap-6 items-start w-full">
      ...
      <div className={`w-full lg:w-[380px] 3xl:w-[440px] 4xl:w-[520px] shrink-0 pr-0 lg:pr-2 space-y-5 select-none ${!showLeftPanel ? 'hidden' : ''}`}>
        ...
        <div className="space-y-5">
          ...
          <Select label="↔️ Angled-face alignment (turned / tilted)" info="For faces that are turned OR tilted — left, right, up, down. Frontal faces are bit-identical in every mode. Fitting the 5 keypoints to a fixed frontal template goes wrong two ways off-frontal: at high yaw both eyes project to nearly the same point so the fit is ill-conditioned in rotation, and at any pitch the eye→mouth distance foreshortens so the template stretches the crop instead. 'stabilize' takes the crop rotation from the eye→mouth axis, which stops head NOD leaking into in-plane roll (±25° of nod at 90° yaw otherwise swings the crop ~30°); it fades in from 40° off-axis and is a TEMPORAL fix — judge it on video, not on a still. 'pose' is the fuller fix: it solves the actual head yaw AND pitch from the keypoints and projects the reference head at that pose. The number to care about is crop scale — with the fixed template the pasted face BREATHES 1.39× in size as the head turns and nods, which reads as both misalignment and per-frame wobble; 'pose' holds it to 1.07×. It fades in from 15° off-axis rather than switching at a threshold, because a threshold flickers (a head parked near it pops between two crops frame to frame). Geometrically cleaner, but OFF by default and worth A/B-ing per clip rather than leaving on. It shipped on for a day and was reported worse on real footage, and the reason is measurable: the pose it solves fits ONE reference head, and in 5 keypoints most of the yaw signal is carried by how far the nose stands out. A person whose nose is 40% more prominent than that head reads 30° of turn as 45°, and 15° as 25°; a flatter nose reads 45° as 17°. That is a per-person systematic error, not frame noise, so it cannot be smoothed away — and the two controls below inherit the same number." value={p.yaw_align || 'off'} onChange={(v) => set('yaw_align', v)} options={['off', 'stabilize', 'pose']} />
          <Toggle label="↔️ Angle: trim hidden surface" info="The second layer of the same angle correction as the selector above, and it only does anything while that is set to 'pose' — it is placed by that alignment's template. The paste mask is built from a canonical ellipse and the convex hull of the 106 landmarks, and both describe a FRONTAL face's footprint. Neither knows a turned head hides half its own face, and the 106-pt model predicts the far-side contour whether or not it is behind the skull, so on a profile the mask reaches back over hair and neck and the swap gets pasted there — that is where 'why is the temple discoloured on profiles' comes from. This projects the reference head at the solved pose, keeps the surface still facing the camera, and trims the mask to it. Measured, this is a PITCH correction and not a yaw one: on a LEVEL head it trims exactly nothing at any yaw, out to an 88° profile. Everything it removes is the under-chin / lower-face region the landmark hull over-claims once the head TILTS — 4.0% at pitch −20, up to 10.2% at pitch −40, mean 2.5% over the pose grid. So it helps the chin-up near-profile case and does nothing for a level turned head. Small — it is not the fix for 'profiles distort' (that is the fade below). It also cannot be made much tighter: the shape comes from a REFERENCE head, and real heads differ from it by more than the over-coverage being removed, so the safety margin eats most of what the trim would reclaim. Cheap (one polygon fill in a space the mask is already built in, no extra warp), and against a head shaped like the reference it never cuts real face — zero truly-visible landmarks trimmed at any pose tested. Against a real one it can: it is placed from the same pose solve as the selector above, so on a prominent-nosed face it is drawn for a pose 15° further round than the person actually is, and it then trims the far side of a face that is entirely visible. That is what 'only half the face got swapped, and the two halves have different texture' is. It is now capped so it can never remove more than a quarter of the mask, which turns that failure into a soft edge rather than a split face — but the cap is a bound, not a fix, which is why this is off by default." checked={!!p.angle_visibility_mask} onChange={(v) => set('angle_visibility_mask', v)} />
          <Slider label="↔️ Angle: extreme-angle fade" info="The third layer, and the one that actually bounds how bad a profile can look. Every swap model here is trained on near-frontal aligned crops; past roughly 55° off-axis the crop is outside that distribution and the model stops reconstructing and starts inventing. That is why hyperswap, hififace and inswapper each fail DIFFERENTLY at different angles, and why no alignment work fixes it: the 5-point fit's residual against the training template grows 44→85px on a 512 crop from frontal to profile, and it does so in EVERY alignment mode, because no similarity transform can map a profile onto a frontal template. So this does not try to make the model right — it fades the swapped crop back toward the original footage as the pose leaves the range the models can serve, which is the one lever that behaves identically for all 13 swappers. This value is the ceiling reached at 90° off-axis; the fade starts at 60° and ramps smoothly (a hard gate on a noisy per-frame pose flickers). 0 = off, and off by default. It is applied from the OUTSIDE IN — the swap keeps full strength over the middle of the face and is withdrawn from the edge inward as the pose gets worse — rather than as a straight cross-dissolve of the whole crop. That distinction is the whole reason it is usable: a cross-dissolve superimposes two differently-shaped faces, so at half strength you see two sets of eyes, two noses and two mouths, which is exactly the 'double halo on lateral poses' artefact. Withdrawing from the edge blends skin into skin instead and never doubles a feature. Raise it if extreme angles still distort, lower it if profiles lose too much likeness." min={0} max={100} step={5} value={num(p.angle_fade_strength, 0)} onChange={(v) => set('angle_fade_strength', v)} />
          ...
          <Select label="Post-processing enhancer" info="The single most expensive stage in a render — about 36% of total time — so this choice costs more than any other on this panel. Measured on an RTX 4070, TensorRT FP16, one 512² face, isolated, engines warm: GPEN-256 3.0ms, GFPGAN v1.4 11.8ms, GPEN-512 19.5ms, RestoreFormer++ 22.7ms, CodeFormer 23.9ms. CodeFormer and RestoreFormer++ look tied but are not: interleaved A/B over 8 rounds put RestoreFormer++ ahead 8/8 by 5.4%, with under 1% spread within each. Because the swap phase runs the GPU at ~98%, those ratios carry straight through to wall clock against an enhance stage that is ~36% of a render: GFPGAN is roughly HALF the cost of RestoreFormer++ and worth ~17% off the whole render, GPEN-512 ~5%, and CodeFormer COSTS you ~2%. That is a quality trade, not a free win — GFPGAN is the older restorer and tends to look smoother and less detailed, while RestoreFormer++ and CodeFormer hold more texture. Cost is measured; which one looks better on your footage is yours to judge. GPEN 1024/2048 are heavier still and not in this comparison. GPEN 256 goes the other way — a quarter of 512's pixels through the network, so it is the cheapest restorer here; it is meant for distant or small faces, comparison grids and preview scrubbing, where 512's extra detail is thrown away by the paste-back downscale anyway. Its output is resized back to the crop size, so it is a drop-in for any of the others." value={p.selected_enhancer} onChange={(v) => set('selected_enhancer', v)} options={meta.enhancers} />
        ...
        {/* DeepFaceLab's merger knobs. Grouped and collapsed by default: they
            are all neutral at 0 and most renders never need them, but when a
            swap looks "pasted" rather than wrong, the fix is usually in here
            rather than in a bigger model. All five run on the CPU over a crop
            already in memory, so they do not compete with the GPU stages. */}
        <Section title="Merger post-processing" collapsible defaultOpen={false}>
          <Slider label="Face size" info="Grows or shrinks the swapped face about its own centre. Every identity swapper keeps the TARGET's head size and bone structure, so when the source person's face is visibly narrower or broader there is otherwise no lever for it — the swap comes out the right identity at the wrong size. 0 = off. This is free: it changes the paste matrix rather than adding a pass, so it costs 0 ms. Note that 'Restore original mouth' re-composites using the target's untouched geometry, so large values here will not register with it." min={-0.2} max={0.2} step={0.01} value={num(p.output_face_scale, 0)} onChange={(v) => set('output_face_scale', v)} />
          <Slider label="Grain match" info="Adds noise matched to the original footage's own measured noise floor. This fixes the most common 'it looks pasted' failure and it is the opposite of what people expect: the face is too CLEAN. The generator emits smooth skin, so on grainy, high-ISO or heavily compressed footage the swap reads as fake precisely because nothing else in the frame is that clean. The level is measured from the plate (median absolute high-pass, so pores and hair do not inflate it), not guessed. 0 = off; try 0.5–1.0 on noisy source. Measured ~6.8 ms per 512² face on CPU." min={0} max={1} step={0.05} value={num(p.merger_grain_match, 0)} onChange={(v) => set('merger_grain_match', v)} />
          <Slider label="Sharpen / soften" info="Signed unsharp mask on the finished face. Positive sharpens; NEGATIVE softens, which is the denoise direction and useful when the enhancer has left crunchy over-detailed skin. 0 = off. Radius scales with crop size, so the effect looks the same at 256/512/1024. The cheapest control here at ~0.6 ms per face." min={-1} max={1} step={0.05} value={num(p.merger_sharpen, 0)} onChange={(v) => set('merger_sharpen', v)} />
          <Slider label="Motion blur" info="Blurs the face along the direction the camera actually smeared it. On fast movement a pin-sharp face on a motion-blurred head is a loud tell. You set the amount; the DIRECTION is measured off the original crop rather than asked for — motion blur destroys detail along the axis of travel, so the plate's own gradient structure points the kernel. 0 = off. Measured ~7.5 ms per 512² face — the dearest of these five." min={0} max={1} step={0.05} value={num(p.merger_motion_blur, 0)} onChange={(v) => set('merger_motion_blur', v)} />
          <Slider label="Histogram match" info="Matches the face's per-channel histogram to the original crop. A fourth colour family alongside the Color/lighting match above: RCT/LCT/MKL match statistical moments (mean, covariance), this matches the shape of the whole distribution — which is what catches a face that is subtly flatter or more contrasted than the plate rather than differently tinted. Works alongside the setting above, not instead of it. 0 = off. Measured ~5.0 ms per 512² face." min={0} max={1} step={0.05} value={num(p.merger_hist_match, 0)} onChange={(v) => set('merger_hist_match', v)} />
          <Slider label="Degrade" info="Bicubic down-and-up on the finished face, to match a soft or heavily compressed plate. A crisp 512px swap dropped into 480p-grade or bitrate-starved footage is sharper than anything around it. 0 = off; at 1 the crop makes a round trip through 40% scale. Measured ~0.8 ms per face." min={0} max={1} step={0.05} value={num(p.merger_degrade, 0)} onChange={(v) => set('merger_degrade', v)} />
        </Section>
