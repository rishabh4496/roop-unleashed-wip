    ...
    <div className="flex flex-col lg:flex-row gap-6 items-start w-full">
      ...
      <div className={`w-full lg:w-[380px] 3xl:w-[440px] 4xl:w-[520px] shrink-0 pr-0 lg:pr-2 space-y-5 select-none ${(progress.processing || !showLeftPanel) ? 'hidden' : ''}`}>
        ...
        <div className="space-y-5">
          ...
          <Select label="Post-processing enhancer" info="The single most expensive stage in a render — about 36% of total time — so this choice costs more than any other on this panel. Measured on an RTX 4070, TensorRT FP16, one 512² face, isolated (±15% run to run): GFPGAN v1.4 ~12ms, GPEN-512 ~18ms, CodeFormer ~22ms, RestoreFormer++ ~23ms. Because the swap phase runs the GPU at ~98%, those ratios carry straight through: GFPGAN is roughly HALF the cost of RestoreFormer++, worth ~18% off the whole render, and GPEN-512 ~8%. That is a quality trade, not a free win — GFPGAN is the older restorer and tends to look smoother and less detailed, while RestoreFormer++ and CodeFormer hold more texture. Cost is measured; which one looks better on your footage is yours to judge. GPEN 1024/2048 are heavier still and not in this comparison. GPEN 256 goes the other way — a quarter of 512's pixels through the network, so it is the cheapest restorer here; it is meant for distant or small faces, comparison grids and preview scrubbing, where 512's extra detail is thrown away by the paste-back downscale anyway. Its output is resized back to the crop size, so it is a drop-in for any of the others." value={p.selected_enhancer} onChange={(v) => set('selected_enhancer', v)} options={meta.enhancers} />
