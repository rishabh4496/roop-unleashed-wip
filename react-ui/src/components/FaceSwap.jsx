    ...
    <div className="flex flex-col lg:flex-row gap-6 items-start w-full">
      ...
      <div className={`w-full lg:w-[380px] 3xl:w-[440px] 4xl:w-[520px] shrink-0 pr-0 lg:pr-2 space-y-5 select-none ${!showLeftPanel ? 'hidden' : ''}`}>
        ...
        <Section title="Video parameters">
          ...
          <Toggle label="Temporal detection (anti-flicker)" info="Video (In-Memory method): one tracked detection pre-pass over the clip. Short misses (up to 10 frames) are interpolated, and a confirmed track is held for up to 3 more missed frames by default, so the swap can't blink out. With 'Stabilize face' also on, keypoints AND mask/mouth landmarks are smoothed per person. The swap pass then skips per-frame detection and stays multi-threaded. Includes identity locking when that toggle is on." checked={!!p.temporal_detection} onChange={(v) => set('temporal_detection', v)} />
