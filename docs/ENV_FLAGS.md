...
| `ROOP_MASK_POOL` | min(2, detmask pool) | Contexts per **selected mask engine**. Capped at 2 because that is where the mask curve ends (RTX 4070, TRT FP16: BiSeNet 82.9 → 98.1 → 97.9 calls/s at 1/2/4 contexts, XSeg-3 412 → 535 → 462) while every context costs VRAM once per selected engine (BiSeNet 192 MB, XSeg ~80 MB) — with `mask_engine` + `mask_engine_2` both set, the 4/4 tier used to hold 2×4 mask contexts for the throughput of 2×2. |
...
| `ROOP_BATCH_SWAP_MAX` | = tiles per face | Batch the swapper's TensorRT profile is built for, and the most the cross-frame batcher will coalesce. Defaults to one face's pixel-boost tiles (1 for a 256px model at 256px, 4 for inswapper at 256px), which is all a pooled context ever receives from its worker. TensorRT reserves every context's activations for the profile's max, so width is paid `ROOP_TRT_POOL` times over: hyperswap @256 measured 520 MB per context at max 1 against 1760 MB at max 10, +5 GB across a 4-wide pool — the difference between fitting a 12GB card and paging over PCIe. Set it (e.g. 4) only to let the batcher coalesce across frames; below 2 the batcher stands down. Changing it builds a new engine once (`<model>_batch<N>` in the cache). |
...
| `ROOP_TEMPORAL_STEP` | 1 (**launcher sets `auto`**) | Scan stride for the "Analyzing faces" pre-pass. `auto` uses a maximum stride of 2, but compares cheap 160×90 motion thumbnails and forces a real detector call when the scene or any tracked face moves; only quiet frames are linearly filled. This keeps the step=1 quality path through head turns while roughly halving detector work in stable shots. `1` forces an unconditional every-frame scan; numeric values retain the fixed-stride behavior. Capped at `ROOP_TEMPORAL_GAP`. |
