...
| `ROOP_TEMPORAL_STEP` | 1 (**launcher sets `auto`**) | Scan stride for the "Analyzing faces" pre-pass. `auto` uses a maximum stride of 2. Motion in 160×90 thumbnails, a profile, adjacent faces, or a known missed track forces fresh detection. Only quiet frames with separated, frontal observations may be skipped. Difficult footage therefore pays for more detection work. `1` forces an unconditional every-frame scan; numeric values retain the fixed-stride behavior. Capped at `ROOP_TEMPORAL_GAP`. |
